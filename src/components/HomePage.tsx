...
const HomePage: React.FC = () => {
  ...
  const currentEpicGames = epicGames.slice(indexOfFirstEpicGame, indexOfLastEpicGame);

  const indexOfLastPrimeGame = currentPrimePage * gamesPerPage;
  const indexOfFirstPrimeGame = indexOfLastPrimeGame - gamesPerPage;
  const currentPrimeGames = primeGames.slice(indexOfFirstPrimeGame, indexOfLastPrimeGame);

  const indexOfLastSteamGame = currentSteamPage * gamesPerPage;
  const indexOfFirstSteamGame = indexOfLastSteamGame - gamesPerPage;
  const currentSteamGames = steamGames.slice(indexOfFirstSteamGame, indexOfLastSteamGame);
  ...
  const scrollToSection = (ref: React.RefObject<HTMLDivElement>) => {
    if (ref.current) {
      window.scrollTo({
        top: ref.current.offsetTop - 64, // Adjust offset for navbar height
        behavior: 'smooth',
      });
    }
  };
  ...
  return (
    <div className='content-wrapper' style={{ paddingTop: '64px' }}>
      {/* Render Paginated Game Sections */}
      <div id="epic" ref={epicRef} > 
